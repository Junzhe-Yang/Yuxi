...
class BaseAgent:
    ...
    def project_stream_message(self, message, state_values: dict, *, context=None):
        """Project one live model message for clients without mutating graph state."""
        del state_values, context
        return message
    ...
    async def stream_messages(self, messages: list[str], input_context=None, **kwargs):
        ...
        async for msg, metadata in graph.astream(
            ...
            msg = self.project_stream_message(msg, {}, context=context)
            yield msg, metadata
    ...
    async def stream_messages_with_state(self, messages: list[str], input_context=None, **kwargs):
        ...
        async for mode, payload in graph.astream(
            ...
            if mode == "values" and isinstance(payload, dict):
                latest_graph_values = payload
            elif mode == "messages":
                msg, metadata = payload
                payload = (
                    self.project_stream_message(
                        msg,
                        latest_graph_values,
                        context=context,
                    ),
