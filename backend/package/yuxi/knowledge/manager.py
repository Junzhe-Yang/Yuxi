...
class KnowledgeBaseManager:
    ...
    async def aembed_texts(
        ...
        """使用知识库自身的 embedding 配置批量编码文本。"""
        kb_instance = await self._get_kb_for_database(db_id)
        embed_texts = getattr(kb_instance, "aembed_texts", None)
        if not callable(embed_texts):
            raise ValueError(f"Database {db_id} does not support text embedding")
        return await embed_texts(db_id, texts)
