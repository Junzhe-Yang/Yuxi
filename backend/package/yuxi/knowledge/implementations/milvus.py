...
from yuxi.knowledge.base import FileStatus, KBOperationError, KnowledgeBase
...
class MilvusQueryError(RuntimeError):
    """Milvus 查询阶段错误，供需要区分空召回与后端失败的内部调用使用。"""


class MilvusEmbeddingError(MilvusQueryError):
    """查询向量编码失败。"""


class MilvusSearchError(MilvusQueryError):
    """Milvus search/hybrid_search 调用失败。"""
...
def _build_file_filter_expression(
    *,
    file_name: str | None,
    ...
    if file_name:
        safe_file_name = _escape_milvus_string(file_name)
        if "%" not in safe_file_name:
            expressions.append(f'source like "%{safe_file_name}%"')
        else:
            expressions.append(f'source like "{safe_file_name}"')
    if filter_file_ids is not None:
        if not filter_file_ids:
            raise ValueError("filter_file_ids 不能为空列表")
        if not all(isinstance(value, str) and value for value in filter_file_ids):
            raise ValueError("filter_file_ids 必须只包含非空字符串")
        literals = ", ".join(f'"{_escape_milvus_string(value)}"' for value in dict.fromkeys(filter_file_ids))
        expressions.append(f"file_id in [{literals}]")
...
class MilvusKB(KnowledgeBase):
    ...
    def _validate_embedding(
        ...
        try:
            ...
            raise ValueError("query_embedding 必须是非空有限数值向量") from exc
        if not values or not all(math.isfinite(value) for value in values):
            raise ValueError("query_embedding 必须是非空有限数值向量")
        expected_dimension_raw = self.databases_meta[db_id].get("embed_info", {}).get("dimension")
        try:
            expected_dimension = int(expected_dimension_raw)
        except (TypeError, ValueError):
            expected_dimension = 0
        if expected_dimension > 0 and len(values) != expected_dimension:
            raise ValueError(f"query_embedding 维度不匹配：expected={expected_dimension}, actual={len(values)}")
        return values
    ...
    async def aembed_texts(
        ...
        except Exception as exc:  # noqa: BLE001 - preserve provider error as cause
            raise MilvusEmbeddingError(f"Embedding texts failed for database {db_id}") from exc
        if len(embeddings) != len(texts):
            raise MilvusEmbeddingError(
                "Embedding provider returned unexpected vector count: "
                f"expected={len(texts)}, actual={len(embeddings)}"
            )
        return [self._validate_embedding(db_id, value) for value in embeddings]
    ...
    async def aquery(self, query_text: str, db_id: str, agent_call: bool = False, **kwargs) -> list[dict]:
        ...
        raise_on_error = bool(merged_kwargs.pop("raise_on_error", False))
        use_async_embedding = bool(merged_kwargs.pop("use_async_embedding", False))
        supplied_embedding = merged_kwargs.pop("query_embedding", None)
        filter_file_ids = merged_kwargs.pop("filter_file_ids", None)
        ...
        try:
            ...
            file_expr = _build_file_filter_expression(
            ...
            async def encode_query() -> list[list[float]]:
                if supplied_embedding is not None:
                    if not isinstance(supplied_embedding, list):
                        raise ValueError("query_embedding 必须是数值列表")
                    return [self._validate_embedding(db_id, supplied_embedding)]
                embed_info = self.databases_meta[db_id].get("embed_info", {})
                try:
                    if use_async_embedding:
                        return await self.aembed_texts(db_id, [query_text])
                    embedding_function = self._get_embedding_function(embed_info)
                    embeddings = embedding_function([query_text])
                    return [self._validate_embedding(db_id, embeddings[0])]
                except Exception as exc:  # noqa: BLE001 - preserve provider error as cause
                    if isinstance(exc, MilvusEmbeddingError):
                        raise
                    raise MilvusEmbeddingError(f"Embedding query failed for database {db_id}") from exc
            ...
            if search_mode == "vector":
                ...
                except Exception as exc:  # noqa: BLE001 - preserve pymilvus error as cause
                    raise MilvusSearchError(f"Milvus vector search failed for database {db_id}") from exc
                ...
                try:
                    ...
                    raise MilvusSearchError(f"Milvus keyword search failed for database {db_id}") from exc
                ...
                try:
                    ...
                    raise MilvusSearchError(f"Milvus hybrid search failed for database {db_id}") from exc
                if results and len(results) > 0 and len(results[0]) > 0:
                    for hit in results[0]:
    ...
    async def get_file_content(self, db_id: str, file_id: str) -> dict:
        ...
        if collection:
            try:
                # 分页读取文档的全部 chunks，避免一次性 query 的 limit
                # 静默截断大文件。
                safe_file_id = _escape_milvus_string(file_id)
                ...
                raise KBOperationError(
                    f"Failed to get file content from Milvus: {e}"
                ) from e
